...
# mp, added the ability to batch through all the models in a folder of models
...
def main(argv):
    model_paths = FLAGS.model
    input_dir  = FLAGS.input
    output_dir = FLAGS.output
    scale_param = FLAGS.scale
    offset_param = FLAGS.offset
    ts_files = find_files(model_paths, ".ts")
    print(ts_files)

    for count, ts_file in enumerate(ts_files):
        model_path=ts_files[count]
...
def find_files(directory, extension):
    ...
    for root, dirs, filenames in os.walk(directory):
        for filename in filenames:
            if not filename.startswith('.') and not filename.endswith('_streaming.ts') and filename.endswith(extension):
